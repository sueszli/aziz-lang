...
class AzizLexer(Lexer[AzizTokenKind]):
    ...
    def _consume_regex(self, regex: re.Pattern[str]) -> re.Match[str] | None:
        """Consume characters matching the given regex pattern."""
    ...
    def _consume_whitespace(self) -> None:
        ...
        def consume_regex(regex: re.Pattern[str]) -> re.Match[str] | None:
            match = regex.match(self.input.content, self.pos)
            if match is None:
                return None
            self.pos = match.end()
        ...
        consume_regex(whitespace_regex)

    def _peek_chars(self, size: int = 1) -> str | None:
        def is_in_bounds(size: Position = 1) -> bool:
            return self.pos + size - 1 < self.input.len

        if not is_in_bounds(size):
            return None
